...
internal sealed class TicketSummarizer(IServiceScopeFactory serviceScopeFactory) : ITicketSummarizer
{
    ...
    private static readonly TokenBucketRateLimiter RateLimiter = new(new()
    {
    ...
    public void UpdateSummary(Guid ticketId, bool enforceRateLimit)
    {
        if (enforceRateLimit)
        {
    ...
    private async Task UpdateSummaryAsync(Guid ticketId)
    {
        ...
        if (ticket is not null)
        {
            ...
            await dbContext.Tickets.Where(t => t.Id == ticketId)
